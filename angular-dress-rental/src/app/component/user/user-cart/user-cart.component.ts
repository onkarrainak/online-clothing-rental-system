...
export class UserCartComponent implements OnInit {
        ...
        if (!!res && Array.isArray(res)) {
          ...
          if (this.cartList.length > 0) {
            this.cartList.map((item: Cart) => {
              let price = item?.mrpPrice * item?.quantity;
              if (item?.duration === '1 Week') {
                price = price * 7;
              }
  ...
  getTotal(quantity: number = 0, mrpPrice: number = 0, duration?:string): number {
    let price = quantity * mrpPrice;
    if (duration === "1 Week") {
      price*=7;
    }
    return price;
